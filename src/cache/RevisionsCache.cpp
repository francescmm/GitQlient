...
void RevisionsCache::setup(
    const WipRevisionInfo& wipInfo,
    const QList<QByteArray>& commits)
{
    ...
    QLog_Debug(
        "Git",
        QString("Configuring the cache for {%1} elements.").arg(totalCommits));
    ...
    insertWipRevision(
        wipInfo.parentSha, wipInfo.diffIndex, wipInfo.diffIndexCached);
...
CommitInfo RevisionsCache::getCommitInfoByRow(int row)
{
    ...
    const auto commit = row >= 0 && row < mCommits.count() ? mCommits.at(row)
                                                           : nullptr;
...
int RevisionsCache::getCommitPos(const QString& sha)
{
    ...
    const auto iter = std::find_if(
        mCommitsMap.begin(),
        mCommitsMap.end(),
        [sha](const CommitInfo& commit) {
            return commit.sha().startsWith(sha);
        });
...
CommitInfo RevisionsCache::getCommitInfoByField(
    CommitInfo::Field field,
    const QString& text,
    int startingPoint)
{
    QMutexLocker lock(&mMutex);
    ...
    if (!sha.isEmpty()) {
        ...
        if (!c.isValid()) {
            ...
            const auto it = std::find_if(
                shas.cbegin(), shas.cend(), [sha](const QString& shaToCompare) {
                    return shaToCompare.startsWith(sha);
                });
...
RevisionFiles RevisionsCache::getRevisionFile(
    const QString& sha1,
    const QString& sha2) const
{
...
void RevisionsCache::insertWipRevision(
    const QString& parentSha,
    const QString& diffIndex,
    const QString& diffIndexCache)
{
    QLog_Debug(
        "Git",
        QString("Updating the WIP commit. The actual parent has SHA {%1}.")
            .arg(parentSha));
    ...
    const auto log = fakeRevFile.count() == mUntrackedfiles.count()
        ? QString("No local changes")
...
bool RevisionsCache::insertRevisionFile(
    const QString& sha1,
    const QString& sha2,
    const RevisionFiles& file)
{
    const auto key = qMakePair(sha1, sha2);

    if (!sha1.isEmpty() && !sha2.isEmpty()
        && mRevisionFilesMap.value(key) != file) {
        QLog_Debug(
            "Git",
            QString("Adding the revisions files between {%1} and {%2}.")
                .arg(sha1, sha2));
...
void RevisionsCache::insertReference(
    const QString& sha,
    ...
    const QString& reference)
{
    QMutexLocker lock(&mMutex);
    QLog_Debug(
        "Git", QString("Adding a new reference with SHA {%1}.").arg(sha));
...
void RevisionsCache::insertLocalBranchDistances(
    const QString& name,
    const LocalBranchDistances& distances)
{
    mLocalBranchDistances[name] = distances;
}

void RevisionsCache::updateWipCommit(
    const QString& parentSha,
    const QString& diffIndex,
    const QString& diffIndexCache)
{
...
bool RevisionsCache::containsRevisionFile(
    const QString& sha1,
    const QString& sha2) const
{
...
RevisionFiles RevisionsCache::parseDiffFormat(
    const QString& buf,
    FileNamesLoader& fl)
{
    RevisionFiles rf;
...
QVector<QPair<QString, QStringList>> RevisionsCache::getBranches(
    References::Type type)
{
    QMutexLocker lock(&mMutex);
    ...
    for (auto commit : mReferences)
        branches.append(QPair<QString, QStringList>(
            commit->sha(), commit->getReferences(type)));
...
void RevisionsCache::setExtStatus(
    RevisionFiles& rf,
    const QString& rowSt,
    int parNum,
    FileNamesLoader& fl)
{
    ...
    const QString extStatusInfo(
        orig + " --> " + dest + " (" + QString::number(type.toInt()) + "%)");
...
QVector<CommitInfo*>::const_iterator RevisionsCache::searchCommit(
    CommitInfo::Field field,
    const QString& text,
    const int startingPoint) const
{
    return std::find_if(
        mCommits.constBegin() + startingPoint,
        mCommits.constEnd(),
        [field, text](CommitInfo* info) {
...
int RevisionsCache::count() const { return mCommits.count(); }

RevisionFiles RevisionsCache::fakeWorkDirRevFile(
    const QString& diffIndex,
    const QString& diffIndexCache)
{
    FileNamesLoader fl;
...
void RevisionsCache::setUntrackedFilesList(
    const QVector<QString>& untrackedFiles)
{
